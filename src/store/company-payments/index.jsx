...
export default function CompanyPayment() {
    ...
    const handleSubmit=(event)=>{
        ...
        axios.post('http://127.0.0.1:8000/api/store/companyPayment',formData, config)
        .then(response => {
    ...
    const fetchcompany = async () => {
        await axios.get('http://127.0.0.1:8000/api/store/companyPayment',config).then(({ data }) => {
            setPayments(data);
            totalAmount=0;
            data.map((item,index)=>{
                totalAmount+=item.amount;
    ...
    return(
        <div className='p-2'>
            ...
            <Link to="/company/payment/create" className='btn btn-outline-success'>{t("create")}</Link>
                        ...
                        {payments.map((item,index)=>{
                            return(
                                <tr> 
                                    ...
                                    <td><Link to="/company/payment/edit" state={{ data: item }} type="button" class="btn btn-success">{t("edit")}</Link></td>
                                </tr>
                            )
                        })}
                    </tbody>
