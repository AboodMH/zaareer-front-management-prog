    ...
    return(
        <div className='p-2'>  
            <div class="d-flex justify-content-between mb-2" style={{alignItems:"center"}}>
                <Link to="/company/create" className='btn btn-outline-success'>{t("create")}</Link>
            </div>
            <div className="row row-cols-auto center">
                {company.map((item,index)=>{
                    return(
                        <Link to="/company/details" state={{ data: item }} style={{textDecorationLine:"none",color:"black"}}>
                            <div className="card mb-3" style={{width:"17.9rem"}}>
                                <img height="200px" src={`http://127.0.0.1:8000/storage/storeCompany/image/${item.image}`} alt='لا يوجد صوره لعرضها'/>
