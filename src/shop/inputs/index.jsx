...
export default function ShopInput({shopName}){
    ...
    return(
        <>
            {editDescription(shopName,editDescriptionIsOpen, setEditDescriptionIsOpen,description,setDescription,t)}
            <div style={{display:"flex",justifyContent:"space-between",padding:"10px"}}>
            <Link to={`/shop/${shopName}/input/create`}className='btn btn-outline-success'>{t("create")}</Link>
...
function editDescription(shopName,editDescriptionIsOpen, setEditDescriptionIsOpen,description,setDescription,t){
    const edit = async(e)=>{
        e.preventDefault();
        const formData = new FormData();
        ...
        await axios.post(`http://127.0.0.1:8000/api/shop/${shopName}/totalInput/${editDescriptionIsOpen[1].id}`, formData,config)
        .then(({data})=>{
