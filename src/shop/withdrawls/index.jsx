...
export default function ShopWithdrawl({shopName}){
    const date=new Date();
    ...
    const [dateWithdrawl, setDateWithdrawl] = useState('');
    ...
    const fetchStaff = async () => {
        await axios.get(`http://127.0.0.1:8000/api/shop/${shopName}/staff`,config).then(({ data }) => {
            setStaff(data);
        });
    }
    ...
    const handleSubmit=(event)=>{
        ...
        axios.post(`http://127.0.0.1:8000/api/shop/${shopName}/withdrawl`,formData, config)
        .then(response => {
    ...
    const fetchThisWithdrawl = async () => {
        await axios.get(`http://127.0.0.1:8000/api/shop/${shopName}/withdrawl`,config).then(({ data }) => {
            setWithdrawls(data);
            totalAmount=0;
            data.map((item,index)=>{
                totalAmount+=item.amount;
    ...
    return(
        <>
            {addWithdrawl(shopName,addWithdrawlIsOpen, setAddWithdrawlIsOpen,dateWithdrawl,setDateWithdrawl,withdrawlAmount,withdrawlDescription,setWithdrawlAmount,setWithdrawlDescription,employeeName, setEmployeeName,staff,t)}
            {editWithdrawl(shopName,editWithdrawlIsOpen, setEditWithdrawlIsOpen,dateWithdrawl,setDateWithdrawl,withdrawlAmount,withdrawlDescription,setWithdrawlAmount,setWithdrawlDescription,employeeName, setEmployeeName,staff,t)}
                            ...
                            withdrawls.map((item, index) => {
                                return (
                                    <tr key={index}>
                                        ...
                                        <td><button onClick={()=>{setEditWithdrawlIsOpen([true,item]);setDateWithdrawl(`${item.month}-${item.day}`);setWithdrawlAmount(item.amount);setWithdrawlDescription(item.description);setEmployeeName(item.employee_name)}} className='btn btn-success'>{t("edit")}</button></td>
                                    </tr>
                                );
                            })
                        }
...
function addWithdrawl(shopName,addWithdrawlIsOpen, setAddWithdrawlIsOpen,dateWithdrawl,setDateWithdrawl,withdrawlAmount,withdrawlDescription,setWithdrawlAmount,setWithdrawlDescription,employeeName,setEmployeeName,staff,t){
    const createWithdrawl = async(e)=>{
        e.preventDefault();
        const formData = new FormData();
        formData.append('date',dateWithdrawl);
        ...
        await axios.post(`http://127.0.0.1:8000/api/shop/${shopName}/withdrawl`, formData,config)
        .then(({data})=>{
                ...
                <form onSubmit={createWithdrawl}>
                    <div className="form-group row" style={{display:"flex",justifyContent:"center"}}>
                        <div className="col-sm-10 mb-4">
                            <input type="date" className="form-control" onChange={(e)=>{setDateWithdrawl(e.target.value)}}/>
                        </div>
                        <div className="col-sm-10 mb-4">
...
function editWithdrawl(shopName,editWithdrawlIsOpen, setEditWithdrawlIsOpen,dateWithdrawl,setDateWithdrawl,withdrawlAmount,withdrawlDescription,setWithdrawlAmount,setWithdrawlDescription,employeeName,setEmployeeName,staff,t){
    const editWithdrawl = async(e)=>{
        ...
        await axios.post(`http://127.0.0.1:8000/api/shop/${shopName}/withdrawl/${editWithdrawlIsOpen[1].id}`, formData,config)
        .then(({data})=>{
            console.log(data.message);
            window.location.reload();
        }).catch(({response})=>{
        ...
        >
            <div className='col'>
                ...
                <form onSubmit={editWithdrawl}>
                    <div className="form-group row" style={{display:"flex",justifyContent:"center"}}>
                        <div className="col-sm-10 mb-4">
                            <input type="date" className="form-control" onChange={(e)=>{setDateWithdrawl(e.target.value)}} value={dateWithdrawl}/>
                        </div>
                        <div className="col-sm-10 mb-4">
