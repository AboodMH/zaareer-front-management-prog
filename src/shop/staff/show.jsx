...
export default function ShopDetailStaff({shopName}){
    ...
    const handleSubmit=(event)=>{
        ...
        axios.post(`http://127.0.0.1:8000/api/shop/${shopName}/staff/${employeeData.employee_id}`,formData, config)
        .then(response => {
    ...
    const fetchThisEmployeeData = async () => {
        await axios.get(`http://127.0.0.1:8000/api/shop/${shopName}/staff/${employeeData.employee_id}`,config).then(({ data }) => {setWithdrawl(data['withdrawl']);setWorkHour(data['workHour']);employeeSalary(data['withdrawl'],data['workHour'],month,employeeData)});
    }
    useEffect(() => {
    ...
    return (
        <div className='p-1'>
            {employeeDataTable(shopName,employeeData,t)}
           {employeeSalaryTable(componentRef,month,setMonth,handleSubmit,t)}
        </div>
    );
}

function employeeDataTable(shopName,employeeData,t){
    return (
        <>
            <img class="mb-3" src={`http://127.0.0.1:8000/storage/shop/${shopName}/shopStaff/image/${employeeData.image}`} alt='لا يوجد صوره لعرضها' height="100px"/>
                ...
                <table class="table table-striped table-bordered table-hover" id="shopTable">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>{total_salary}</td>
                            <td><Link to={`/shop/${shopName}/staff/edit`} state={{ data: employeeData }} type="button" class="btn btn-success" style={{height: "auto"}}>{t("edit")}</Link></td>
                        </tr>
                    </tbody>
                </table>
            </div>
